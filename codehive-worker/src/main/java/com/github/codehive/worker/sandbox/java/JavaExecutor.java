...
public class JavaExecutor implements LanguageExecutor {
    ...
    private static final String COMPILE_IMAGE =
    ...
    static {
        String profile = null;
        try (
            InputStream is = JavaExecutor.class.getResourceAsStream(
                "/seccomp/sandbox-profile.json"
            )
        ) {
            if (is != null) profile = new String(
                is.readAllBytes(),
    ...
    private void ensureImageExists() {
        for (String image : List.of(COMPILE_IMAGE, EXEC_IMAGE)) {
            try {
    ...
    @Override
    public ContainerSession prepare(
        ...
        memoryLimitMb =
            memoryLimitMb != null ? memoryLimitMb : DEFAULT_MEMORY_LIMIT_MB;
        ...
        if (compileResult != null) {
            ...
            return ContainerSession.compilationError(
                compileResult.getCompilationError()
            );
        }
        ...
        HostConfig hostConfig = HostConfig.newHostConfig()
            ...
            .withTmpFs(
                Map.of(
                    "/tmp",
                    "size=33554432,noexec,nosuid,mode=1777",
                    "/run",
                    "size=8388608,noexec,nosuid,mode=755"
                )
            )
            .withUlimits(new Ulimit[] {
                new Ulimit("fsize", 33554432L, 33554432L),
            })
            .withSecurityOpts(buildSecurityOpts());

        CreateContainerResponse container = dockerClient
            .createContainerCmd(EXEC_IMAGE)
    ...
    public ExecutionResult runTestCase(
        ...
        String timeoutCmd = String.format(
            "timeout --kill-after=2s %.3fs java Main",
            timeLimitSec
        );
                ...
                new ResultCallback.Adapter<Frame>() {
                    ...
                    public void onNext(Frame frame) {
                        ...
                        synchronized (sharedBytes) {
                            ...
                            try {
                                if (
                                    frame.getStreamType() == StreamType.STDOUT
                                ) {
        ...
        if (exitCode == 137L) {
            return ExecutionResult.memoryLimitExceeded(
                session.getMemoryLimitMb()
            );
        }
        if (truncated[0]) {
            return ExecutionResult.outputLimitExceeded(
                stdout + "\n[Output truncated: exceeded 4 MB limit]",
                executionTime
            );
        }
        if (exitCode != 0) {
    ...
    public void cleanup(ContainerSession session) {
        if (session.getContainerId() != null) {
            try {
                dockerClient
                    .removeContainerCmd(session.getContainerId())
                    .withForce(true)
                    .exec();
            } catch (Exception e) {
                logger.warn(
    ...
    private void cleanupBetweenRuns(ContainerSession session) {
        ...
        try {
            String cleanExecId = dockerClient
                .execCreateCmd(session.getContainerId())
    ...
    private ExecutionResult compile(Path workDir) {
        ...
        try {
            HostConfig hostConfig = HostConfig.newHostConfig()
                .withBinds(
                    new Bind(workDir.toString(), new Volume("/workspace"))
                )
                ...
                .withTmpFs(
                    Map.of(
                        "/tmp",
                        "size=67108864,noexec,nosuid,mode=1777",
                        "/run",
                        "size=8388608,noexec,nosuid,mode=755"
                    )
                )
                .withUlimits(new Ulimit[] {
                    new Ulimit("fsize", 67108864L, 67108864L),
                })
                .withSecurityOpts(buildSecurityOpts());

            CreateContainerResponse container = dockerClient
                .createContainerCmd(COMPILE_IMAGE)
                .withHostConfig(hostConfig)
                .withWorkingDir("/workspace")
                .withUser("nobody")
                ...
                .exec(
                    new ResultCallback.Adapter<Frame>() {
                        ...
                        public void onNext(Frame frame) {
                            ...
                            if (payload == null) return;
                            int remaining =
                                COMPILE_STDERR_LIMIT_BYTES - bytesWritten[0];
            ...
            Files.walkFileTree(
                ...
                new SimpleFileVisitor<Path>() {
                    ...
                    public FileVisitResult visitFile(
                        ...
                        try {
                            if (
                                file.toRealPath().startsWith(realBase)
