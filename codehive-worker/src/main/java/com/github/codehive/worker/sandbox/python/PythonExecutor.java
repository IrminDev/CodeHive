...
public class PythonExecutor implements LanguageExecutor {
    ...
    static {
        String profile = null;
        try (
            InputStream is = PythonExecutor.class.getResourceAsStream(
    ...
    private void ensureImageExists() {
        try {
            dockerClient.inspectImageCmd(EXEC_IMAGE).exec();
            logger.info("Docker image {} is available", EXEC_IMAGE);
        } catch (Exception e) {
            logger.info("Pulling Docker image {}...", EXEC_IMAGE);
            try {
                dockerClient.pullImageCmd(EXEC_IMAGE).start().awaitCompletion();
                logger.info("Successfully pulled image {}", EXEC_IMAGE);
    ...
    public ExecutionResult runTestCase(
        ...
        String timeoutCmd = String.format(
            "timeout --kill-after=2s %.3fs python main.py",
            timeLimitSec
        );
        ...
        if (exitCode != 0 && stderr.contains("SyntaxError")) {
