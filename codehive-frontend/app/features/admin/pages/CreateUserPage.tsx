...
export function CreateUserPage() {
  ...
  return <AdminShell active="users" breadcrumbs={[{ label: "Admin", to: "/admin" }, { label: "Users", to: "/admin/users" }, { label: "Create" }]} contentClassName="max-w-3xl">
    ...
    <form onSubmit={(event) => void submit(event)} className={`${panelClass} p-5 sm:p-7 space-y-5`}>
      <Field label="Role"><select className={inputClass} value={selectedRole ?? ""} onChange={(event) => setRole(event.target.value as Role)} required>{roles.map((item) => <option key={item} value={item}>{item[0] + item.slice(1).toLowerCase()}</option>)}</select></Field>
