  ...
  return <AdminShell active="overview" breadcrumbs={[{ label: "Admin" }, { label: "Overview" }]}>
    ...
    {!canAnalytics ? <PermissionDashboard /> : loading ? <AdminLoading rows={5} /> : error || !data ? <AdminError message={error ?? "Statistics unavailable."} onRetry={() => load()} /> : <div className="space-y-6">
      <div className={`${panelClass} p-4 flex flex-col md:flex-row md:items-center gap-3 justify-between`}>
        ...
        <div className="flex flex-wrap items-end gap-2">{[7, 30, 90].map((days) => <button key={days} onClick={() => selectRange(days)} className="px-3 py-2 rounded-lg text-xs font-medium border border-gray-200 dark:border-gray-700 hover:text-azure dark:hover:text-yellow">{days} days</button>)}<label className="grid gap-1 text-[10px] text-gray-500">From<CalendarInput size="compact" value={from?.slice(0, 10) ?? ""} onChange={(date) => { const next = new URLSearchParams(params); const value = startOfLocalDay(date); value ? next.set("from", value) : next.delete("from"); setParams(next); }} className="w-44" /></label><label className="grid gap-1 text-[10px] text-gray-500">Through<CalendarInput size="compact" value={to ? new Date(new Date(to).getTime() - 1).toISOString().slice(0, 10) : ""} onChange={(date) => { const next = new URLSearchParams(params); const value = endExclusiveLocalDay(date); value ? next.set("to", value) : next.delete("to"); setParams(next); }} className="w-44" /></label><button onClick={() => setParams({})} className="px-3 py-2 rounded-lg text-xs font-medium border border-gray-200 dark:border-gray-700 hover:text-azure dark:hover:text-yellow">Default</button></div>
      </div>
