  ...
  return <AdminShell active="incidents" breadcrumbs={[{ label: "Admin", to: "/admin" }, { label: "Incidents" }]}>
    ...
    <div className="space-y-4">
      <div className={`${panelClass} p-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4`}>
        ...
        <label><FilterLabel text="Method" /><select className={`${inputClass} py-2.5`} value={params.get("method") ?? ""} onChange={(event) => update("method", event.target.value)}><option value="">All methods</option>{["GET", "POST", "PATCH", "PUT", "DELETE"].map((method) => <option key={method}>{method}</option>)}</select></label>
        <label><FilterLabel text="From" /><input type="date" className={`${inputClass} py-2.5`} value={fromDate} onChange={(event) => update("from", startOfLocalDay(event.target.value))} /></label>
        <label><FilterLabel text="Through" /><input type="date" className={`${inputClass} py-2.5`} value={toDate} onChange={(event) => update("to", endExclusiveLocalDay(event.target.value))} /></label>
        <label><FilterLabel text="Page size" /><select className={`${inputClass} py-2.5`} value={params.get("size") ?? "20"} onChange={(event) => update("size", event.target.value)}><option value="20">20 rows</option><option value="50">50 rows</option><option value="100">100 rows</option></select></label>
      </div>
