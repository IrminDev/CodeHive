...
export function TeacherNotificationSettingsPage() {
  ...
  return <TeacherShell active="notifications" breadcrumbs={[{ label: "Teacher", to: "/teacher" }, { label: "Notifications" }]}>
    ...
    {loading ? <TeacherLoading rows={4} /> : error && !settings ? <TeacherError message={error} onRetry={() => void load()} /> : settings && <div className="space-y-6">
      ...
      <section className={`${panelClass} overflow-hidden ${settings.emailEnabled ? "" : "opacity-60"}`}><header className="px-5 py-4 border-b border-gray-100 dark:border-gray-800/60"><div className="flex items-center gap-2"><BellRing size={16} className="text-azure dark:text-yellow" /><h2 className="font-semibold">Email types</h2></div></header><div className="divide-y divide-gray-100 dark:divide-gray-800/60">{settings.preferences.map((preference) => <div key={preference.type} className="px-5 py-4 flex flex-col sm:flex-row sm:items-center gap-4"><div className="flex-1 min-w-0"><h3 className="text-sm font-medium">{label(preference.type)}</h3><p className="mt-1 text-xs text-gray-500">{descriptions[preference.type] ?? "Email notification for this teaching event."}</p></div>{preference.reminderLeadMinutes != null && <label className="flex items-center gap-2 text-xs text-gray-500"><Clock3 size={13} /><select disabled={!settings.emailEnabled || !preference.enabled} value={preference.reminderLeadMinutes} onChange={(event) => patchPreference(preference.type, { reminderLeadMinutes: Number(event.target.value) })} className={`${inputClass} py-2 px-2.5 text-xs`}>{REMINDERS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>}<Toggle checked={preference.enabled} disabled={!settings.emailEnabled} onChange={(enabled) => patchPreference(preference.type, { enabled })} label={`Enable ${label(preference.type)}`} /></div>)}</div></section>
