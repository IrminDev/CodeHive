...
class AdminUserControllerIntegrationTest {
    ...
    @Test
    void administratorCanGrantAuditLogScopeToAnotherAdministrator() throws Exception {
        User targetAdmin = userRepository.save(new User("Audit", "Admin", "ADM-007", "audit-admin@example.com",
                passwordEncoder.encode("Pass123!"), Role.ADMIN));

        mockMvc.perform(patch("/api/admin/users/{id}/scopes", targetAdmin.getId())
                        .header("Authorization", "Bearer " + adminToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"grant\":[\"VIEW_AUDIT_LOG\"],\"revoke\":[],"
                                + "\"reason\":\"Grant audit log access\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.user.scopes[?(@ == 'VIEW_AUDIT_LOG')]").isNotEmpty());

        org.assertj.core.api.Assertions.assertThat(userRepository.findById(targetAdmin.getId()).orElseThrow().getScopes())
                .contains(Scope.VIEW_AUDIT_LOG);
    }
